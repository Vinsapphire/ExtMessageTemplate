// From  https://github.com/christocracy/RoutedApp
...
Ext.define('Ext.ux.router.Router', {
    ...
    constructor: function(config) {
        ...
        this.addEvents('beforeurlrecognize', 'beforedispatch', 'dispatch', 'redirect');
	...
	*/
	...
	dispatch: function(url, params) {
		if( this.fireEvent('beforeurlrecognize', url) === false ) {
			return;
		}
		...
		if( this.route.controller != 'app' ) {
			var modulePath = '';
			// add module to controller search
			if( typeof this.route.module != 'undefined' ) {
				modulePath = this.route.module.toLowerCase() + '.';
				
				// if request without controller
				if( typeof this.route.controller == 'undefined' ) {
					// set controller to default module controller
					this.route.controller = Ext.String.capitalize( this.route.module ) + 'Base';
					modulePath = '';
				}
			}
			...
			// find controller
			this.controller = this.app.getController( modulePath + Ext.String.capitalize( this.route.controller ) );
		}

		
		// set controller action (e.g. request /user/add will fire actionAdd() on User controller)
		this.route.action = 'action' + Ext.String.capitalize( this.route.action );
